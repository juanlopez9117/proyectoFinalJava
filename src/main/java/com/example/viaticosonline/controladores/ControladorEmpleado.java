...
public class ControladorEmpleado {
    ...
    public ResponseEntity<?> registrar(@RequestBody Empleado empleado){
        try{
            ...
            return ResponseEntity.status(HttpStatus.OK).body(servicioEmpleado.registrar(empleado));
    ...
    public ResponseEntity<?> buscarEmpleado(){
        try{
            return ResponseEntity.status(HttpStatus.OK).body(servicioEmpleado.buscarTodos());
        }catch (Exception error){
